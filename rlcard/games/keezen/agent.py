...
from rlcard.games.keezen.move import Move
...
class RuleBasedMoveEvaluator:
    ...
    def get_marbles_in_path(fields, fields_with_marbles, with_color=None, without_color=None) -> {}:
        """Returns a dictionary with the indexes of a path with a marble on it."""
        result = {}
        index = 0
        for field in fields:
...
class RuleBasedAgentAdapter:
    """Adapter class to use the RuleBasedAgent in RLCard. The state and actions are converted GameState and moves."""
    ...
    def eval_step(self, state):
        ...
        if not move:
            return 0, {}
        raw_action = move.get_raw_action(self.rule_based_agent.game, game_state)
        action_index = GameActions.ALL_ACTIONS_271.index(raw_action)
        info = {}
